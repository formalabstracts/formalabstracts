...
informally ::
  ...
  Let X be a compact Hausdorff space and let S be a subset
  of C(X, C) which separates points. Then the complex unital
  ``*``-algebra generated by S is dense in C(X, C).
...
preformally ::
   ...
   forevery (A : star-subalgebra C),
   (separates-points A) ∧ (unital A) ∧ (closed A) → (A = C)

For wiki's definition, `C``*``-algebra <https://en.wikipedia.org/wiki/C*-algebra>`_.
...
* The complex unital ``*``-algebra generated by S consists of the algebra generated by S, 1, and closed
under conjugation.
